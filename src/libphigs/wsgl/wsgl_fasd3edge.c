   ...
   )
{
   ...
   if (eflag == PEDGE_VISIBILITY) {
      wsgl_begin_emu(GL_LINES);
      for (i = 0; i < edata->num_edges - 1; i++) {
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(points[i].x,
                       points[i].y,
                       points[i].z);
            wsgl_vertex3f_emu(points[i + 1].x,
                       points[i + 1].y,
                       points[i + 1].z);
            if (record_geom){
   ...
   )
{
   ...
   if (eflag == PEDGE_VISIBILITY) {
      ...
      for (i = 0; i < edata->num_edges - 1; i++) {
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(ptcolrs[i].point.x,
                       ptcolrs[i].point.y,
                       ptcolrs[i].point.z);
            wsgl_vertex3f_emu(ptcolrs[i + 1].point.x,
                       ptcolrs[i + 1].point.y,
                       ptcolrs[i + 1].point.z);
            if (record_geom){
      ...
      if (edata->num_edges < num_vertices) {
         ...
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(ptcolrs[i].point.x,
                       ptcolrs[i].point.y,
                       ptcolrs[i].point.z);
            wsgl_vertex3f_emu(ptcolrs[0].point.x,
                       ptcolrs[0].point.y,
                       ptcolrs[0].point.z);
            if (record_geom){
              vertex_indices[n_vertices] = wsgl_add_vertex(ptcolrs[i].point.x,
   ...
   )
{
   ...
   if (eflag == PEDGE_VISIBILITY) {
      ...
      for (i = 0; i < edata->num_edges - 1; i++) {
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(ptnorms[i].point.x,
                       ptnorms[i].point.y,
                       ptnorms[i].point.z);
            wsgl_vertex3f_emu(ptnorms[i + 1].point.x,
                       ptnorms[i + 1].point.y,
                       ptnorms[i + 1].point.z);
            if (record_geom){
      ...
      if (edata->num_edges < num_vertices) {
         ...
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(ptnorms[i].point.x,
                       ptnorms[i].point.y,
                       ptnorms[i].point.z);
            wsgl_vertex3f_emu(ptnorms[0].point.x,
                       ptnorms[0].point.y,
                       ptnorms[0].point.z);
            if (record_geom){
              vertex_indices[n_vertices] = wsgl_add_vertex(ptnorms[i].point.x,
   ...
   )
{
   ...
   if (eflag == PEDGE_VISIBILITY) {
      ...
      for (i = 0; i < edata->num_edges - 1; i++) {
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(ptconorms[i].point.x,
                       ptconorms[i].point.y,
                       ptconorms[i].point.z);
            wsgl_vertex3f_emu(ptconorms[i + 1].point.x,
      ...
      if (edata->num_edges < num_vertices) {
         ...
         if (edata->edgedata.edges[i] == PEDGE_ON) {
            wsgl_vertex3f_emu(ptconorms[i].point.x,
                       ptconorms[i].point.y,
                       ptconorms[i].point.z);
            wsgl_vertex3f_emu(ptconorms[0].point.x,
                       ptconorms[0].point.y,
                       ptconorms[0].point.z);
            if (record_geom){
              vertex_indices[n_vertices] = wsgl_add_vertex(ptconorms[i].point.x,
